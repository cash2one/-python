...
def parse_detail(title, date, url, content, filename):
    ...
    for each_json in content:
        ...
        if 'ue86.com' in PyQuery(each_json).text():
            if u'】' != PyQuery(each_json).text().strip()[-1] and u'】' in PyQuery(each_json).text():
        ...
        if u'相关阅读' in each or u'扫一扫' in each or u'相关链接' in each or u'天道提示' in each:
            break
        #print each
        if each.strip()[1] == u'、' or '<strong>' in PyQuery(each_json).html() or (each.strip()[3] == ':' if len(each.strip()) > 3 else False) or each.strip()[-1] == u'】':
            flag_abstract = False
            flag_method = True
        if flag_abstract:
            steps.append(each)
